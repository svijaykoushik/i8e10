...
const FilterModal: FC<FilterModalProps> = ({ isOpen, onClose, onApply, onExport, initialFilter, wallets }) => {
  const [modalFilter, setModalFilter] = useState<FilterState>(initialFilter);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Filter Transactions" footer={footer}>
      <div className="space-y-4">
        ...
        <div>
            <label htmlFor="filter-type" className={labelClasses}>Transaction Type / பரிவர்த்தனை வகை</label>
            <select
                id="filter-type"
                value={modalFilter.transactionType}
                onChange={(e) => handleFilterChange('transactionType', e.target.value as TransactionFilterType)}
                className={inputClasses}
            >
                <option value={TransactionFilterType.ALL}>All Types / அனைத்தும்</option>
                <option value={TransactionFilterType.INCOME}>Income / வரவு</option>
                <option value={TransactionFilterType.EXPENSE}>Expense / செலவு</option>
                <option value={TransactionFilterType.TRANSFER}>Transfers / பரிமாற்றங்கள்</option>
                <option value={TransactionFilterType.ADJUSTMENT}>Adjustments / சரிசெய்தல்கள்</option>
            </select>
        </div>
        <div>
